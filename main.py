...
def get_literature_keys(literature, member):
    ...
    raw_cat = pd.Categorical(member, categories=["owned", "new", "recommended"], ordered=False)
    data["member"] = raw_cat
...
def add_literature(all_papers, relationships, newPaper):
    ...
    relation = {'from': availablePaper.loc[0,'paperID'], 'to': newPaper.loc[0,'paperID']}
    relationships = relationships.append(relation, ignore_index=True)

    # add referenced paper to all papers 
    if any(all_papers['paperID'].isin(newPaper['paperID'])): # IF referenced paper is already saved
        # count occurence of paper up by 1
        all_papers.loc[all_papers['paperID'] == newPaper.loc[0, 'paperID'], 'occurence'] += 1
    else:
        # add referenced paper to all papers
...
for entries in range(len(bib_database.entries)):
    ...
    if any(all_papers['paperID'].isin(availablePaper['paperID'])): # IF available paper is already saved
        # count occurence of paper up by 1
        all_papers.loc[all_papers['paperID'] == availablePaper.loc[0, 'paperID'], 'occurence'] += 1
        # change membership to owned
        all_papers.loc[all_papers['paperID'] == availablePaper.loc[0, 'paperID'], 'member'] = 'owned'
    else:
        # add available paper to all papers
    ...
    for ref in range(len(resp.json()['references'])):  
        ...
        all_papers, relationships = add_literature(all_papers, relationships, referencedPaper)
    ...
    for ref in range(len(resp.json()['citations'])):
        ...
        all_papers, relationships = add_literature(all_papers, relationships, cited_papers)
...
idx_delete_papers = np.flatnonzero(relationships['from'].isin(delete_papers['paperID']) | relationships['to'].isin(delete_papers['paperID']))
...
new_paper = all_papers[(all_papers['occurence'] >= all_papers['occurence'].quantile(0.9)) & (all_papers['member'] == "new")]

# change membership to recommended
all_papers.loc[all_papers['paperID'].isin(new_paper['paperID']), 'member'] = 'recommended'
...
nx.draw(G , pos=position, width=0.5, node_size=nodeSize, node_color=node_colors, edge_color='gray', 
     cmap=colors)
